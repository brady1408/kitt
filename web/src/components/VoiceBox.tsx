...
export function Scanner({ active }: { active: boolean }) {
  return (
    <div className="relative h-1.5 w-full overflow-hidden bg-secondary" aria-label={active ? "Processing" : "System ready"}>
      <div className={active ? "kitt-sweep absolute top-0 h-full w-[22%] bg-primary shadow-signal" : "absolute inset-y-0 left-0 w-1/3 bg-primary/45"} />
    </div>
  );
}
